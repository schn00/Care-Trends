/**
 * Care Trends — Worker entry point
 *
 * Handles POST /api/token and hands every other request to the static files
 * in /public. Same origin for both, which is what lets the sign-in cookie work.
 *
 * WHAT THIS DOES
 ...
 *
 * REQUIRED VARIABLES (Cloudflare dashboard -> your Worker -> Settings ->
 * Variables and Secrets)
 *   GOOGLE_CLIENT_ID      the OAuth client id (also in index.html; public)   [Text]
 *   GOOGLE_CLIENT_SECRET  the OAuth client secret                            [Secret]
 *   COOKIE_KEY            32+ random characters, your own                    [Secret]
...
const setCookie = (value) =>
  `${COOKIE_NAME}=${encodeURIComponent(value)}; Path=${COOKIE_PATH}; Max-Age=${COOKIE_MAX_AGE}; HttpOnly; Secure; SameSite=Lax`;
const clearCookie = () =>
...
/* Same-origin only. The app and this code share a domain, so a request from
   anywhere else has no business here. */
...
async function handleToken(request, env) {
  ...
  /* Name the specific ones, not the whole list. Plain vars are wiped by every
     deploy unless they are declared in wrangler.toml, so GOOGLE_CLIENT_ID
     going missing on its own is a common and confusing failure. */
  const missing = ["GOOGLE_CLIENT_ID", "GOOGLE_CLIENT_SECRET", "COOKIE_KEY"].filter((k) => !env[k]);
  if (missing.length) {
    const hint = missing.length === 1 && missing[0] === "GOOGLE_CLIENT_ID"
      ? " — plain variables are cleared on every deploy, so this one belongs in wrangler.toml under [vars], not the dashboard"
      : " — set these under Settings → Variables and Secrets, then deploy again";
    return json({ error: "server_not_configured", detail: "missing: " + missing.join(", ") + hint }, 500);
  }
...
export default {
  async fetch(request, env) {
    ...
    /* TEMPORARY DIAGNOSTIC — remove once sync is working.
       Lists the NAMES of everything this Worker can see at runtime. Never
       prints a value, only whether one is present and how long it is, which
       is enough to spot a typo, a variable saved in the wrong place, or a
       value that is empty. Open it in a browser: /api/diag */
    if (url.pathname === "/api/diag") {
      const expected = ["GOOGLE_CLIENT_ID", "GOOGLE_CLIENT_SECRET", "COOKIE_KEY"];
      const seen = {};
      for (const k of Object.keys(env || {})) {
        const v = env[k];
        seen[k] = typeof v === "string" ? "string, length " + v.length : typeof v;
      }
      return json({
        note: "names only, no values",
        all_bindings_visible_to_worker: seen,
        expected: expected.map((k) => ({
          name: k,
          present: !!env[k],
          type: typeof env[k],
    ...
    /* Anything else is the website itself. env.ASSETS is the static-files
       binding declared in wrangler.toml. */
    if (url.pathname.startsWith("/api/")) {
      return json({ error: "not_found", detail: "no such endpoint" }, 404);
    }
